...
export default function SimpleScriptGenerator() {
  ...
  const loadProducts = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setError((errorData as any).error || 'Failed to load products');
        setLoading(false);
        return;
      }
  ...
  const generateEducationalHook = (category: string) => {
  ...
  const generateScript = (product: Product): GeneratedScript => {
...
NARRATION: Real soil health builds thriving underground ecosystems.`,
      waveSpeedScenes: [
        { scene: 1, duration: '3s', visuals: 'Close-up soil', textOverlay: hook },
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: 32, fontFamily: 'system-ui' }}>
      ...
      <p style={{ color: '#666', marginBottom: 32 }}>Generate scripts from your Google Sheet</p>
        ...
        {products.length > 0 && (
          <div style={{ marginTop: 16 }}>
            ...
            <div style={{ marginTop: 12, maxHeight: 200, overflowY: 'auto', fontSize: 13 }}>
              {products.map((p) => (
                <div key={p.id} style={{ padding: 8, borderBottom: '1px solid #e5e7eb' }}>
      ...
      {products.length > 0 && scripts.length === 0 && (
        ...
        >
          ...
          <p style={{ fontSize: 14, color: '#666', marginBottom: 16 }}>
            Create educational scripts for all {products.length} products
          </p>
          ...
          <div style={{ display: 'grid', gap: 24 }}>
            {scripts.map((script) => (
              <div
                ...
                style={{ background: 'white', border: '2px solid #e5e7eb', borderRadius: 8, overflow: 'hidden' }}
              >
                <div
                  style={{
                    background: '#2b6f3f',
                ...
                <div style={{ padding: 16 }}>
                  ...
                  <details style={{ marginTop: 16 }}>
                    <summary
                      style={{ cursor: 'pointer', fontWeight: 600, padding: 12, background: '#eff6ff', borderRadius: 6 }}
                    >
                      ...
                      {script.waveSpeedScenes.map((scene, i) => (
                        ...
                        >
                          ...
                          >
                            ...
                            <span
                              style={{ fontSize: 12, background: '#dbeafe', padding: '2px 8px', borderRadius: 12 }}
                            >
                              {scene.duration}
                            </span>
                          </div>
                          <div style={{ fontSize: 13, color: '#374151', marginTop: 4 }}>🎬 {scene.visuals}</div>
                          <div style={{ fontSize: 13, color: '#6b7280', marginTop: 4 }}>📝 "{scene.textOverlay}"</div>
                          {scene.productShot && (
                            <div style={{ marginTop: 8, fontSize: 12, fontWeight: 600, color: '#059669' }}>
      ...
      {products.length === 0 && !loading && (
        ...
        >
          ...
          <p style={{ fontSize: 14 }}>This will read your Google Sheet and prepare scripts</p>
        </div>
      )}
    </div>
  );

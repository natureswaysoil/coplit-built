...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  let query = supabaseAdmin.from('inventory_alert_subscriptions').select('*').order('created_at', { ascending: false })
  if (email && typeof email === 'string') query = query.eq('email', email)
